...
function CreateApiService($http, dataService, globalConst) {
    ...
    this.getToken = (userName, password) => {
        ...
        return $http.post(`/api/auth`, JSON.stringify(userDetails)).then((resp) => {
            dataService.token = resp.data
        })
    }

    this.ListenAndReadFromCompaniesRequestsQueue = () => {
         $http.get(globalConst.get_companies_requests).then((resp) => {
        })
    }

    this.getAllItems = (getMatchingFlightsurl) => {
        //Get All Countries.
            ...
            //Get All Flights
                    .then(() => {
                        return $http.get(getMatchingFlightsurl)
                            .then((flights) => {
