    ...
    public class LoggedInAirlineFacade : AnonymousUserFacade, ILoggedInAirlineFacade
    {
        ...
        public void CancelFlight(LoginToken<AirlineCompany> token, Flight flight)
        {
            if (UserIsValid(token) && flight != null)
            {
                if (token.User.Id == flight.AirlineCompany_Id)
                {
                    ...
                    //ImHere_backgroundDAO.WriteRequestToLog(LogCategories.AirlineCompanies | LogCategories.Flights | LogCategories.Deletions, $"Company: {token.User.UserName} Tried To Cancel Flight From Her Flights List.", true);
                }
                else
                {
                    //ImHere_backgroundDAO.WriteRequestToLog(LogCategories.AirlineCompanies | LogCategories.Flights | LogCategories.Deletions, $"Company: {token.User.UserName} Tried To Cancel Flight From Some Flights List.", false);
                    throw new TicketNotMatchException("Sorry But You Can't Cancel Flight Of Another Comapny.");
                }
            }
            //else
                //ImHere_backgroundDAO.WriteRequestToLog(LogCategories.AirlineCompanies | LogCategories.Flights | LogCategories.Deletions, $"Anonymous User Tried To Cancel Some Flight.", false);
        }
        ...
        public void MofidyAirlineDetails(LoginToken<AirlineCompany> token)
        {
            if (UserIsValid(token))
            {
                ...
                if (airlineUser != null)
                {
                    _userDAO.UpdateUserName(airlineUser.UserName, token.User.UserName);
                    _airlineDAO.Update(token.User);
                    //ImHere_backgroundDAO.WriteRequestToLog(LogCategories.AirlineCompanies | LogCategories.Updates, $"Company: {token.User.UserName} Tried To Update Her Details.", true);

                }
                else
                {
                    //ImHere_backgroundDAO.WriteRequestToLog(LogCategories.AirlineCompanies | LogCategories.Updates, $"Company: {token.User.UserName} Tried To Update Her Details.", false);
                    throw new UserNotExistException($"Sorry, But '{token.User.UserName}' Does Not Exist.");
                }
            }
            //else
                //ImHere_backgroundDAO.WriteRequestToLog(LogCategories.AirlineCompanies | LogCategories.Updates, $"Anonymous User Tried To Update Some Airline Company.", false);
            ...
            if (UserIsValid(token))
            {
                ...
                if (airlineUser != null)
                {
                    if (_userDAO.TryChangePasswordForUser(airlineUser, oldPassword, newPassword))
                    {
                        ...
                        //ImHere_backgroundDAO.WriteRequestToLog(LogCategories.AirlineCompanies | LogCategories.Updates, $"{token.User.UserName} Company Tried To Change Her Password.", true);
                    }
                    else
                    {
                        //ImHere_backgroundDAO.WriteRequestToLog(LogCategories.AirlineCompanies | LogCategories.Updates, $"{token.User.UserName} Company Tried To Change Her Password.", false);
                        throw new WrongPasswordException("Your Old Password Is Incorrect!");
                    }
                }
            }
            //else
                //ImHere_backgroundDAO.WriteRequestToLog(LogCategories.AirlineCompanies | LogCategories.Updates, $"Anonymous User Tried To Change Password For Some Airline Company. Id: {token.User.Id} ({token.User.UserName}).", false);
        }
        ...
        public void UpdateFlight(LoginToken<AirlineCompany> token, Flight flight)
        {
            if (UserIsValid(token) && flight != null)
            {
                if (token.User.Id == flight.AirlineCompany_Id)
                {
                    ...
                    //ImHere_backgroundDAO.WriteRequestToLog(LogCategories.AirlineCompanies | LogCategories.Flights | LogCategories.Updates, $"Comapny: {token.User.UserName} Tried To UPDATE Some Flight From Her Flights List.", true);
                }
                else
                {
                    //ImHere_backgroundDAO.WriteRequestToLog(LogCategories.AirlineCompanies | LogCategories.Flights | LogCategories.Updates, $"Comapny: {token.User.UserName} Tried To UPDATE Some Flight Of Another Company.", false);
                    throw new FlightNotMatchException($"Sorry, But You Can Update Only Flights Of This Company.");
                }
            }
            //else
                //ImHere_backgroundDAO.WriteRequestToLog(LogCategories.AirlineCompanies | LogCategories.Flights | LogCategories.Updates, $"Anonymous User Tried To UPDATE Some Flight From Some Company.", false);
        }
